...
max @ min @ -   counter1 @ scale * /  1+ DUP .( Delta:) . delta !
...
: indexes  
           BEGIN input_file TO fid get_number tempnumber ! tempnumber @ min @ - delta @ /  tempindex ! tempindex @ ." tepmindex:" . SPACE
                 ...
                 on_this_index @ counter1 @ scale * * tempindex @ + 32 * S>D fid @ REPOSITION-FILE DROP  
                 tempnumber @  S>D (D.)  fid @ WRITE-FILE DROP 
 counter- 0= UNTIL ;
CR
  
indexes 
: get_nth_number scale * 32 * tempindex @ + S>D fid @ REPOSITION-FILE DROP 
                get_number
;
